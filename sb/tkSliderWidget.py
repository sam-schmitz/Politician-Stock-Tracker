...
class Slider_Datetime(Frame):
    ...
    def __init__(
        ...
        width: int = 400,
        height: int = 80,
        min_val: datetime = datetime(2024, 4, 1),
        max_val: datetime = datetime(2024, 4, 2),
        step_size: Optional[float] = None,  #needs fix
        init_lis: Optional[list[datetime]] = None,
        show_value = True,
        removable = False,
        addable = False,
    ):
        ...
        if step_size == None:
            # inherit from class variable
            step_size = self.STEP_SIZE
        if init_lis is not None:
            assert all(isinstance(date, datetime) for date in init_lis)
        
        min_val = int(time.mktime(min_val.timetuple()))
        max_val = int(time.mktime(max_val.timetuple()))
        if init_lis is not None:
            init_lis = [int(time.mktime(dt.timetuple()))for dt in init_lis]
            """for dt in init_lis:
